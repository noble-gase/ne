...
func warpper(opts ...Option) *wrapper {
	wrapper := new(wrapper)
	for _, f := range opts {
		f(wrapper)
	}
...
func TestToInsert(t *testing.T) {
	...
	assert.Equal(t, []any{"og", "M", 29}, args)
	...
	assert.Equal(t, []any{"og", "M", 29, "13605109425"}, args)

	// map 字段顺序不一定
	sql, args, err = warpper(Table("user")).insertSQL(M{
		"age":    29,
		"gender": "M",
		"name":   "og",
	})
	...
	assert.Equal(t, []any{29, "M", "og"}, args)
}
...
func TestToBatchInsert(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, "INSERT INTO user (name, gender, age) VALUES (?, ?, ?), (?, ?, ?)", sql)
	assert.Equal(t, []any{"og", "M", 29, "test", "W", 20}, args)
	...
	assert.Nil(t, err)
	assert.Equal(t, "INSERT INTO user (name, gender, age, phone) VALUES (?, ?, ?, ?), (?, ?, ?, ?)", sql)
	assert.Equal(t, []any{"og", "M", 29, "13605109425", "test", "W", 20, "13605105471"}, args)

	// map 字段顺序不一定
	sql, args, err = warpper(Table("user")).batchInsertSQL([]M{
		{
			"age":    29,
			"gender": "M",
			"name":   "og",
	...
	assert.Nil(t, err)
	assert.Equal(t, "INSERT INTO user (age, gender, name) VALUES (?, ?, ?), (?, ?, ?)", sql)
	assert.Equal(t, []any{29, "M", "og", 20, "W", "test"}, args)
}
...
func TestToUpdate(t *testing.T) {
	...
	assert.Equal(t, []any{"og", "M", 29, 1}, args)
	...
	assert.Equal(t, []any{"og", "M", 29, "13605109425", 1}, args)
	...
	assert.Equal(t, []any{29, "M", "og", 1}, args)
	...
	assert.Equal(t, []any{"og", "M", 29, 1, 2}, args)

	sql, args, err = warpper(
		Table("product"),
		Where("id = ?", 1),
	).updateSQL(M{"price": Expr("price * ? + ?", 2, 100)})
	assert.Nil(t, err)
